...
namespace planning
{
  ...
  // main thread of behavior planner server.
  // invoke PlanCycleCallback loop.
  ...
  void BehaviorPlannerServer::PlanCycleCallback()
  {
    ...
    if (has_updated_map)
    {
      ...
      // if semantic map updated, invoke behavior_planner RunOnce.
      // if success, smm use as bp result behavior and publish plan result.
      TicToc timer; // ???
